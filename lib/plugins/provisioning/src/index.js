...
routes.get(
  ...
  const evaluateFunction = (functionName, args, code) => {
    debug('Evaluating %s function: %s, with arguments %o',
      functionName, code, args);

    const f = xeval(code, { BigNumber: BigNumber });

    // Call the function and store the result in the results cache
    const result = f(...args);
    debug('%s function returned result: %o', functionName, result);
    results[functionName] = result;
    return result;
  };

  const validator = (meteringPlan, ratingPlan, cb) => {
    let errors = [];
    ...
    meteringPlan.metrics.forEach( (meteringMetric) => {
      const measures = extend({}, ...map(meteringPlan.measures, (measure) => {
        return !measure.name ? {} : {
          [measure.name]: 1
        };
      }));
      ...
      try {
        // Evaluate meter function
        if (meteringMetric.meter) {
          args = [measures];
          code  = meteringMetric.meter.toString();
          functionName = 'meter';
          evaluateFunction(functionName, args, code);
        }

        // Evaluate accumulate function
        if(meteringMetric.accumulate) {
          args = [undefined, results['meter'] || 1,
            0, 100, 50, 150, undefined]; // a, qty, start, end, from, to, twCell
          code  = meteringMetric.accumulate.toString();
          functionName = 'accumulate';
          evaluateFunction(functionName, args, code);
        }

        // Evaluate aggregate function
        if(meteringMetric.aggregate) {
          args = [undefined, results['accumulate'] || 0,
            results['accumulate'] || 1,
            undefined, undefined]; // a, prev, curr, aggTwCell, accTwCell
          code = meteringMetric.aggregate.toString();
          functionName = 'aggregate';
          evaluateFunction(functionName, args, code);
        }

        // Evaluate summarize function
        if(meteringMetric.summarize) {
          args = [100, results['aggregate'] || 1, 50, 150]; // t, qty, from, to
          code = meteringMetric.summarize.toString();
          functionName = 'summarize';
          evaluateFunction(functionName, args, code);
        }

        const ratingMetric = ratingPlan.metrics
          .filter(function(ratingMetric) {
            return ratingMetric.name === meteringMetric.name;
          })[0];

        // Evaluate rate function
        if(ratingMetric.rate) {
          args = [0.1, results['aggregate'] || 1]; // price, qty
          code = ratingMetric.rate.toString();
          functionName = 'rate';
          evaluateFunction(functionName, args, code);
        }

        // Evaluate charge function
        if(ratingMetric.charge) {
          // t, qty, from, to
          args = [100, results['rate'] || 1, 50, 150];
          code = ratingMetric.charge.toString();
          functionName = 'charge';
          evaluateFunction(functionName, args, code);
        }
      }
  ...
  const repsonseNotFound = (message) => {
    return {
      status: 404,
      body: {
        message: message
  ...
  routes.post('/v1/activate/resources/:resource_type/plans/:plan_name',
    function *(req) {

      const meteringPlanId = yield mappings.mappedMeteringPlan(
        req.params.resource_type, req.params.plan_name);
      debug('Metering plan ID: %s', meteringPlanId);

      const ratingPlanId = yield mappings.mappedRatingPlan(
        req.params.resource_type, req.params.plan_name);
      debug('Rating plan ID: %s', ratingPlanId);

      if (!meteringPlanId || !ratingPlanId)
        return repsonseNotFound('Plan mappings not found');

      const dbReader = plansDb.read({
        cache: false
      });

      const meteringPlan = yield dbReader['metering'](meteringPlanId);
      const ratingPlan = yield dbReader['rating'](ratingPlanId);

      if (!meteringPlan || !ratingPlan)
        return repsonseNotFound('Plans not found');

      let response;

      validator(meteringPlan, ratingPlan, (errors) => {
        if(errors.length > 0) {
          response = {
            status: 400,
            body: {
