...
require.cache[require.resolve('abacus-debug')].exports
  = spy((namaspace) => loggerSpy);
...
describe('Oauth', () => {
  ...
  context('when credentials are valid', () => {
    ...
    it('should return an authorization header', () => {
      const abacusOauthSpy = mockAbacusOauth('Bearer AAA', undefined);

      expect(oauth.init).to.not.throw(Error);
      expect(oauth.authHeader())
        .to.deep.equal({ authorization: 'Bearer AAA' });
