...
describe('UAA', () => {
  ...
  context('and create UAA client', () => {
    ...
    it('should pass if UAA client is created', (done) => {
      ...
      uaa.createClient(instanceId, (statusCode, response) => {
        ...
        expect(response.credentials.client_id).to.equal(
          config.resourceProviderPrefix(instanceId));

        expect(postSpy.callCount).to.equal(1);
        const body = postSpy.args[0][1].body;
        expect(body.client_id).
          to.equal(config.resourceProviderPrefix(instanceId));
    ...
    it('should fail if UAA server respond with error 500', (done) => {
      ...
      const uaa = require('../auth/uaa.js');
      uaa.createClient(instanceId, (statusCode, credentials) => {
